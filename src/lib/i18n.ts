const translations = {
  en: {
    appTitle: "Gantt",
    newTask: "New task",
    subtask: "Subtask",
    import: "Import",
    export: "Export",
    share: "Share",
    shareCopied: "Copied",
    drawerTitle: "Task",
    taskColumnHeader: "Task",
    labelName: "Name",
    labelStart: "Start",
    labelEnd: "End",
    labelProgress: "Progress",
    labelDuration: "Duration (days)",
    labelPredecessor: "Predecessor",
    placeholderPredecessor: "e.g.: 2FS",
    labelNotes: "Notes",
    placeholderNotes: "Notes (HTML)",
    defaultTaskName: "Task",
    defaultSubtaskName: "Subtask",
    defaultProjectName: "New project",
    defaultBaseTask: "Base task",
    defaultSeedTask1: "Task 1",
    defaultSeedTask2: "Task 2",
    defaultSeedTask3: "Task 3",
    importError: "Could not import. Verify the JSON is compatible with OnlineGantt.",
    invalidGanttFile: "Invalid file. Drop a valid .gantt JSON file.",
    dropToImport: "Drop file to import",
    importFormatError: "JSON format incompatible with OnlineGantt",
    sharePrompt: "Copy this link:",
    readOnlyView: "Read-only view",
    readOnlySharedView: "Read-only shared view",
    selectTaskToEdit: "Select a task to edit.",
    noTasksToShow: "No tasks to show.",
    emptyCreateFirstTask: "Create first task",
    deleteTask: "Delete task",
    confirmDeleteTitle: "Delete task?",
    confirmDeleteDescription: "This action cannot be undone.",
    cancel: "Cancel",
    confirm: "Confirm",
    milestone: "Milestone",
    dateLocale: "en-US",
  },
  es: {
    appTitle: "Gantt",
    newTask: "Nueva tarea",
    subtask: "Subtarea",
    import: "Importar",
    export: "Exportar",
    share: "Compartir",
    shareCopied: "Copiado",
    drawerTitle: "Tarea",
    taskColumnHeader: "Tarea",
    labelName: "Nombre",
    labelStart: "Inicio",
    labelEnd: "Fin",
    labelProgress: "Progreso",
    labelDuration: "Duración (días)",
    labelPredecessor: "Predecesor",
    placeholderPredecessor: "ej: 2FS",
    labelNotes: "Notas",
    placeholderNotes: "Notas (HTML)",
    defaultTaskName: "Tarea",
    defaultSubtaskName: "Subtarea",
    defaultProjectName: "Nuevo proyecto",
    defaultBaseTask: "Tarea base",
    defaultSeedTask1: "Tarea 1",
    defaultSeedTask2: "Tarea 2",
    defaultSeedTask3: "Tarea 3",
    importError: "No se pudo importar. Verifica que el JSON sea compatible con OnlineGantt.",
    invalidGanttFile: "Archivo inválido. Suelta un .gantt JSON válido.",
    dropToImport: "Suelta el archivo para importar",
    importFormatError: "Formato JSON incompatible con OnlineGantt",
    sharePrompt: "Copia este enlace:",
    readOnlyView: "Vista de solo lectura",
    readOnlySharedView: "Vista compartida de solo lectura",
    selectTaskToEdit: "Selecciona una tarea para editar.",
    noTasksToShow: "No hay tareas para mostrar.",
    emptyCreateFirstTask: "Crear primera tarea",
    deleteTask: "Borrar tarea",
    confirmDeleteTitle: "¿Borrar tarea?",
    confirmDeleteDescription: "Esta acción no se puede deshacer.",
    cancel: "Cancelar",
    confirm: "Confirmar",
    milestone: "Hito",
    dateLocale: "es-ES",
  },
} as const

type Locale = keyof typeof translations
type TranslationKey = keyof typeof translations.en

function detectLocale(): Locale {
  const lang = (typeof navigator !== "undefined" && (navigator.language || navigator.languages?.[0])) || "en"
  return lang.toLowerCase().startsWith("es") ? "es" : "en"
}

export const locale: Locale = detectLocale()
export const t = (key: TranslationKey): string => translations[locale][key]
